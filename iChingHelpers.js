...
var iChingHelper = {
  ...
  // 1 byte = 8 bits
  // 3 bytes = 24 bits
  // 24 bits = 4 hexagrams (a hexagram is a 6 bit number)
  ...
  readBytes: function (filePath) {
    ...
    myStream.on('data', (chunk) => {
      ...
      for (const b of chunk) {
        binaryChunk += this.hex2bin(b);
      }    

      sixBitArray = this.getArrayOf6Bits(binaryChunk);

      hexagrams = sixBitArray.map(str => {
        return this.sixBitToSingleIching(str);
      });
  ...
  sixBitToSingleIching: function(bitsString){
    var hex = this.hexagrams[bitsString];
    return hex
      ? hex.hexagram
      : 'Cannot find hexagram for ' + bitsString;
  },

  getArrayOf6Bits: function(bits) {
    let sixBits = [];
...
// iChingHelper.readBytes("/Users/patrick/Desktop/CapraCarYT_QuartzVapor_Instagram.mp4");
